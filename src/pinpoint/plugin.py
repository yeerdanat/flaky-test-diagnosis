"""Pytest plugin that Pinpoint's runner injects into every trial subprocess.

Loaded explicitly with ``-p pinpoint.plugin`` and activated only when
``PINPOINT_OUT`` is set, so it is inert in any other pytest invocation.

Responsibilities (driven entirely by environment variables):

- ``PINPOINT_OUT``       — path of the JSONL results file to append to (activation flag)
- ``PINPOINT_ORDER``     — path of a JSON list of node ids; run exactly these, in this order
- ``PINPOINT_RNG_SEED``  — seed ``random`` (and numpy if present) at session start
- ``PINPOINT_STATEDIFF`` — "1" to snapshot/diff process state around every test
- ``PINPOINT_ROOT``      — repo root; module-global snapshots are limited to modules under it
"""
...
_ENV_IGNORE_PREFIXES = ("PINPOINT_", "PYTEST_")
_REPR_LIMIT = 300
...
def _active() -> bool:
    return bool(os.environ.get("PINPOINT_OUT"))
...
def _snapshot() -> dict:
    root = os.environ.get("PINPOINT_ROOT", "")
    env = {
        k: v
...
def pytest_configure(config):
    if not _active():
        return
    seed = os.environ.get("PINPOINT_RNG_SEED")
    if seed is not None:
        import random
...
def pytest_collection_modifyitems(config, items):
    if not _active():
        return
    order_file = os.environ.get("PINPOINT_ORDER")
    if not order_file:
        return
...
def pytest_runtest_protocol(item, nextitem):
    if not _active():
        yield
        return
    statediff = os.environ.get("PINPOINT_STATEDIFF") == "1"
    ...
    with open(os.environ["PINPOINT_OUT"], "a") as f:
