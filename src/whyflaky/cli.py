"""WhyFlaky CLI.
    ...
    whyflaky scan [path] [--rounds N] [--budget N] [--fix] [--verify] [--json PATH]
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"whyflaky {__version__}")
    ...
    scan.add_argument("--json", metavar="PATH", default=None,
                      help="JSON report path (default: <path>/.whyflaky/report.json)")
    scan.add_argument("--db", metavar="PATH", default=None,
                      help="SQLite history path (default: <path>/.whyflaky/whyflaky.db)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = Path(args.json) if args.json else repo / ".whyflaky" / "report.json"
